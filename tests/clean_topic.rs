...
fn clean_topic_removes_existing_state() {
    ...
    assert!(topic_dir.exists(), "publisher should have created the topic dir");
...
fn clean_topic_refuses_when_publisher_active() {
    ...
    drop(owner);
    cleaner.clean_topic("events").expect("clean after owner-drop should succeed");
...
fn clean_topic_then_republish_works() {
    ...
    let t = std::thread::spawn(move || {
        ...
        let mut sub = sub_bus.subscribe_timeout("events", Duration::from_secs(5)).unwrap();
        sub.recv().unwrap()
    });
