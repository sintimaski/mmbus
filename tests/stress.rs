...
fn fan_out_100k_messages_4_subscribers() {
    ...
    let handles: Vec<_> = (0..SUBS)
        .map(|_| {
            ...
            std::thread::spawn(move || {
                let mut sub =
                    Subscriber::connect("bus", &c, Duration::from_secs(10)).unwrap();
    ...
    pub_.wait_for_subscribers(SUBS, Duration::from_secs(10)).unwrap();
...
fn drop_oldest_50k_messages_3_subscribers() {
                ...
                loop {
                    match sub.receive_timeout(Duration::from_millis(500)) {
                        Ok(Some(bytes)) => {
                            ...
                            assert!(
                                (v as i128) > last,
                                "sub {i}: out-of-order {v} after {last}"
                            );
                            last = v as i128;
                            count += 1;
                            if sleep_us > 0 {
    ...
    for (i, h) in handles.into_iter().enumerate() {
        ...
        assert!(count <= N, "sub {i} got more than published ({count} > {N})");
        eprintln!(
...
fn rapid_publisher_restart_cycles() {
    ...
    for cycle in 0..CYCLES {
        ...
        let sub_thread = std::thread::spawn(move || {
            let mut sub =
                Subscriber::connect("bus", &c, Duration::from_secs(5)).unwrap();
