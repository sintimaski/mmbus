...
fn scenario_a_crash_recovery_preserves_records_each() {
    run_policy("a_each", FsyncPolicy::Each, |cfg| {
        ...
        let mut sub = Subscriber::connect_with(
            "bus",
            cfg,
...
fn scenario_c_retention_drops_oldest_and_surfaces_too_old() {
    let cfg = BusConfig {
        capacity: 16,
        slot_size: 32,
        base_dir: std::env::temp_dir().join("mmbus_wal_acc").join("c_retention"),
        wal: WalConfig {
            enabled: true,
    ...
    assert!(stats.oldest_cursor > 0, "retention must have dropped older cursors");
    drop(p);
    ...
    match Subscriber::connect_with(
        ...
        Err(mmbus::Error::CursorTooOld { requested: 0, oldest }) => {
            assert!(
                oldest > 0,
                "CursorTooOld must surface WAL's oldest, not 0"
            );
        }
