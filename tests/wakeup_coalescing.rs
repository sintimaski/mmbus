...
fn no_missed_wakeup_under_mixed_burst_and_idle() {
    ...
    let consumer = std::thread::spawn(move || {
        ...
        for _ in 0..n {
            ...
            tx.send(u64::from_le_bytes(buf.as_slice().try_into().unwrap())).unwrap();
        }
    });
...
fn coalescing_reduces_wakeups_when_publisher_outpaces_subscriber() {
    ...
    for _ in 0..n {
        rx.recv_timeout(Duration::from_secs(5)).expect("missed wakeup / hang");
    }
    consumer.join().unwrap();
    ...
    assert!(s.wakeups_sent_total > 0, "subscriber must have been woken at least once");
    cleanup(&cfg);
}
