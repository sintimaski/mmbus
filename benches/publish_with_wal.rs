...
fn publish_throughput(c: &mut Criterion) {
    ...
    let configs: Vec<(&str, WalConfig)> = vec![
        ...
        (
            "wal_none",
            WalConfig { enabled: true, fsync_policy: FsyncPolicy::None, ..Default::default() },
        ),
        (
            "wal_batched",
            WalConfig { enabled: true, fsync_policy: FsyncPolicy::Batched, ..Default::default() },
        ),
        (
            "wal_each",
            WalConfig { enabled: true, fsync_policy: FsyncPolicy::Each, ..Default::default() },
        ),
    ];
