...
impl Bus {
    ...
    pub fn subscribe_with_history(
        ...
        self.subscribe_with_history_timeout(topic, n_messages_back, Duration::from_secs(30))
    }
    ...
    pub fn wait_for_subscribers(&mut self, topic: &str, n: usize, timeout: Duration) -> Result<()> {
        if let Some(p) = self.publishers.get_mut(topic) {
    ...
    pub fn slow_subscribers(&self, topic: &str, threshold: u64) -> Vec<(usize, u64)> {
        self.publishers
            .get(topic)
