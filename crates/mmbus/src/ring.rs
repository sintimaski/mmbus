...
impl RingBuffer {
    pub fn create(
        ...
        unsafe {
            ...
            p.add(OFF_SLOT_SIZE)
                .cast::<u32>()
                .write_unaligned(slot_payload_size);
            p.add(OFF_MAX_SUBS)
                .cast::<u32>()
                .write_unaligned(max_subscribers);
    ...
    pub fn set_wakeflag(&self, idx: usize) {
        self.wakeflag_atomic(idx)
            .store(WAKEFLAG_WAITING, Ordering::SeqCst);
    }
    ...
    pub fn release_cursor(&self, idx: usize) {
        self.cursor_atomic(idx)
            .store(CURSOR_UNCLAIMED, Ordering::Release);
    }
