...
impl SegmentWriter {
    ...
    /// in sync.
    pub fn flush_for_external_fsync(&mut self) -> Result<(std::fs::File, u64), WriterError> {
        self.file.flush()?;
...
mod tests {
    ...
    fn append_writes_expected_bytes() {
        ...
        let record_len = u32::from_le_bytes(bytes[32..36].try_into().unwrap()) as usize;
    ...
    fn rejects_out_of_order_cursor() {
        ...
        match w.append(11, 0, b"x") {
            Err(WriterError::CursorRegressed {
                got: 11,
                expected: 10,
            }) => (),
