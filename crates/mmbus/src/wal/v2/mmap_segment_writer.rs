...
pub enum WriterError {
    ...
    #[error("segment_size {segment_size} too small (need at least {min} to hold the header)")]
...
impl MmapSegmentWriter {
    ...
    pub fn create(
        ...
        writer
            .tail_atomic()
            .store(SEGMENT_HEADER_LEN as u64, Ordering::Release);
        Ok(writer)
    }
    ...
    pub fn append(
        ...
        let crc_input = unsafe {
            std::slice::from_raw_parts(
                slot_ptr.add(crc_input_start),
                crc_input_end - crc_input_start,
            )
        };
...
mod tests {
    ...
    fn appended_record_has_writing_bit_clear_after_commit() {
        ...
        assert_eq!(
            record_len & WRITING_BIT_U32,
            0,
            "WRITING_BIT must be clear on commit"
        );
    }
    ...
    fn appended_record_body_round_trips_with_crc_match() {
        ...
        let crc_stored = u32::from_le_bytes(
            bytes[off + record_len - 4..off + record_len]
                .try_into()
                .unwrap(),
        );
    ...
    fn v1_reader_rejects_v2_segment_with_unsupported_version() {
        ...
        match crate::wal::record::SegmentHeader::parse(&bytes[..SEGMENT_HEADER_LEN]) {
            ...
            other => {
                panic!("v1 reader should reject v2 segment with UnsupportedVersion, got {other:?}")
            }
        }
    }
}
