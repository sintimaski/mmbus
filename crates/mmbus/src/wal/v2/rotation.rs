...
pub fn open_segment_reader(
    ...
    crate::wal::v2::mmap_segment_reader::MmapSegmentReader,
    crate::wal::v2::mmap_segment_reader::ReaderError,
> {
...
mod tests {
    ...
    fn rotate_creates_new_segment_and_updates_active_dat() {
        ...
        assert_eq!(
            w.append(1, 0, b"01234567").unwrap(),
        ...
        loop {
            match reader.next_record() {
                ...
                ReadOutcome::EndOfSegment => {
                    ...
                    reader =
                        MmapSegmentReader::open(&segment_path(dir.path(), current_first)).unwrap();
                }
    ...
    fn two_consecutive_rotations_chain_segments() {
        ...
        assert!(matches!(
            w0.append(0, 0, b"01234567").unwrap(),
            AppendOutcome::Ok { .. }
        ));
        let w1 = rotate(dir.path(), &w0, &active, 1, segment_size).unwrap();
        assert_eq!(active.load_first_cursor(), 1);
        ...
        assert!(matches!(
            w1.append(1, 0, b"01234567").unwrap(),
            AppendOutcome::Ok { .. }
        ));
        ...
        for fc in [0u64, 1, 2] {
            assert!(
                segment_path(dir.path(), fc).exists(),
                "missing segment {fc}"
            );
        }
                ...
                loop {
                    match writer.append(i, 0, &payload).unwrap() {
                        ...
                        AppendOutcome::SegmentFull => {
                            ...
                            writer =
                                rotate(&dir_for_writer, &writer, &active_writer, i, segment_size)
                                    .unwrap();
                        }
                    }
                }
