...
impl Wal {
    ...
    pub fn open(dir: &Path, cfg: WalConfig) -> Result<Self, WalError> {
        ...
        let (writer, pending_cursor, active_bytes) =
            if let Some((&last_first_cursor, last_path)) = segments.iter().next_back() {
                ...
                if next_cursor == last_first_cursor {
                    ...
                    let writer =
                        MmapSegmentWriter::create(&new_path, cfg.segment_size_max, next_cursor)?;
                    ...
                    (Some(Arc::new(writer)), next_cursor, bytes)
                }
            } else {
        ...
        let retained_bytes: u64 = if let Some(active_first) = segments
            .iter()
            .next_back()
            .map(|(k, _)| *k)
            .filter(|_| active_bytes > 0)
        {
    ...
    pub fn current_ts(&self) -> u64 {
        match self.cfg.fsync_policy {
            ...
            FsyncPolicy::None | FsyncPolicy::Each => self
                .wall_base_nanos
    ...
    pub fn append(&self, cursor: u64, ts_unix_nanos: u64, payload: &[u8]) -> Result<(), WalError> {
        if !self.cfg.enabled {
            return Ok(());
        }
        ...
        let writer = match writer_guard.as_ref() {
            Some(w)
                if self.active_bytes.load(Ordering::Acquire) + record_bytes <= segment_size_max =>
            {
                w.clone()
            }
            _ => {
                    ...
                    > segment_size_max
                {
                    ...
                    self.writer
                        .load_full()
                        .expect("writer set by rotate_locked")
                } else {
        ...
        if matches!(self.cfg.fsync_policy, FsyncPolicy::Each) {
            ...
            // on the OS.  Called OUTSIDE the lock so the 3-5 ms
    ...
    fn rotate_locked(&self, inner: &mut Inner, new_first_cursor: u64) -> Result<(), WalError> {
        if let Some(old_arc) = self.writer.swap(None) {
            ...
            self.retained_bytes
                .fetch_add(final_bytes, Ordering::Release);
            // New segment becomes active.
            inner
                .segments
                .insert(new_first_cursor, segment_path(&self.dir, new_first_cursor));
            self.active_bytes
                .store(new_w.current_tail(), Ordering::Release);
    ...
    fn enforce_retention_locked(&self, inner: &mut Inner) -> Result<(), WalError> {
        loop {
            ...
            let active_first = self
                ...
                .unwrap_or(u64::MAX);
            let oldest_first = inner.segments.keys().copied().find(|&k| k != active_first);
            match oldest_first {
                Some(k) => {
...
fn spawn_flusher_thread(
    ...
    thread::spawn(move || {
        ...
        while !shutdown.load(Ordering::Acquire) {
            ...
            let ts = wall_base_nanos.saturating_add(mono_base.elapsed().as_nanos() as u64);
...
mod tests {
    ...
    fn read_from_returns_appended_records() {
        ...
        let w = Wal::open(dir.path(), cfg(4096, 1 << 20)).unwrap();
        for i in 0..5u64 {
            w.append(i, i * 1000, format!("rec-{i}").as_bytes())
                .unwrap();
        }
    ...
    fn read_from_cursor_too_old_after_retention() {
        ...
        assert!(
            oldest > 0,
            "expected retention to prune segment 0, oldest = {oldest}"
        );
        match w.read_from(0) {
    ...
    fn batched_flusher_eventually_advances_durable_cursor() {
        ...
        while w.durable_cursor() < 10 {
            if Instant::now() >= deadline {
                panic!(
                    "flusher never advanced durable_cursor (got {})",
                    w.durable_cursor()
                );
            }
