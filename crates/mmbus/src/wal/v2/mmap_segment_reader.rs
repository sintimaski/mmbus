...
use crate::wal::record::{
    Record, SegmentHeaderError, MAX_RECORD_LEN, RECORD_FRAMING, SEGMENT_HEADER_LEN, SEGMENT_MAGIC,
};
use crate::wal::v2::mmap_segment_writer::{align_record_len, SEGMENT_VERSION_V2, WRITING_BIT_U32};
use memmap2::Mmap;
use std::fs::File;
use std::io;
...
pub enum ReaderError {
    ...
    #[error(
        "record_len {record_len} exceeds MAX_RECORD_LEN ({MAX_RECORD_LEN}) at offset {offset}"
    )]
...
impl MmapSegmentReader {
    ...
    pub fn open(path: &Path) -> Result<Self, ReaderError> {
        ...
        if magic != SEGMENT_MAGIC {
            return Err(ReaderError::Header(SegmentHeaderError::BadMagic {
                got: magic,
            }));
        }
        ...
        if version != SEGMENT_VERSION_V2 {
            return Err(ReaderError::Header(
                SegmentHeaderError::UnsupportedVersion { got: version },
            ));
        }
    ...
    pub fn next_record(&mut self) -> ReadOutcome {
        ...
        let stored_crc = u32::from_le_bytes(body[record_len - 4..record_len].try_into().unwrap());
...
mod tests {
    ...
    fn write_records(path: &std::path::Path, records: &[(u64, &[u8])]) {
        let w =
            MmapSegmentWriter::create(path, 4096, records.first().map(|(c, _)| *c).unwrap_or(0))
                .unwrap();
        for (c, p) in records {
                ...
                match r.next_record() {
                    ...
                    ReadOutcome::AwaitMore => {
                        if Instant::now() >= deadline {
                            panic!("reader timed out at {} / {} records", got.len(), N);
                        }
                        std::hint::spin_loop();
                    }
