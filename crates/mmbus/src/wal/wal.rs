...
impl Wal {
    ...
    pub fn append(&self, cursor: u64, ts_unix_nanos: u64, payload: &[u8]) -> Result<(), WalError> {
        ...
        let record_bytes = crate::wal::record::RECORD_FRAMING as u64 + payload.len() as u64;
...
mod tests {
    ...
    fn read_from_walks_records_in_order() {
        ...
        let records: Vec<_> = wal.read_from(0).unwrap().collect::<Result<_, _>>().unwrap();
    ...
    fn read_from_skips_records_before_requested_cursor() {
        ...
        let records: Vec<_> = wal.read_from(2).unwrap().collect::<Result<_, _>>().unwrap();
    ...
    fn reopen_resumes_after_existing_segments() {
        ...
        assert!(
            s.pending_cursor >= 3,
            "reopen must resume past existing records"
        );
