...
fn bus_api_two_topics_independent() {
    ...
    let mut bus = Bus::with_config("app", cfg.clone());
    bus.wait_for_subscribers("topic-a", 1, Duration::from_secs(5))
        .unwrap();
    bus.wait_for_subscribers("topic-b", 1, Duration::from_secs(5))
        .unwrap();
...
fn bus_api_iterator() {
    ...
    let mut bus = Bus::with_config("app", cfg.clone());
    bus.wait_for_subscribers("stream", 1, Duration::from_secs(5))
        .unwrap();
    for i in 0..n {
        loop {
...
fn bus_api_recv_into_slice_too_large_errors() {
    ...
    bus.wait_for_subscribers("ch", 1, Duration::from_secs(5))
        .unwrap();
    bus.publish("ch", b"this payload is longer than four bytes")
        .unwrap();
