...
fn receive_into_reuses_buffer_capacity() {
    ...
    let handle = std::thread::spawn(move || {
        ...
        assert!(
            grown_cap > starting_cap,
            "buffer must have grown for the first payload"
        );
        ...
        assert_eq!(
            buf.capacity(),
            grown_cap,
            "capacity must not shrink between recvs"
        );
        ...
        assert_eq!(
            buf.capacity(),
            grown_cap,
            "second short recv must reuse the buffer"
        );
    });
...
fn receive_into_clears_buf_on_entry() {
    ...
    let handle = std::thread::spawn(move || {
        ...
        assert_eq!(
            &buf[..],
            b"fresh",
            "previous content must not survive the next receive"
        );
    });
    p.wait_for_subscribers(1, Duration::from_secs(5)).unwrap();
...
fn try_receive_into_returns_false_when_empty() {
    ...
    let handle = std::thread::spawn(move || {
        ...
        assert!(
            buf.is_empty(),
            "buffer must be cleared even on empty try_receive_into"
        );
    });
    p.wait_for_subscribers(1, Duration::from_secs(5)).unwrap();
    handle.join().unwrap();
...
fn receive_timeout_into_signals_timeout_with_false() {
    ...
    let handle = std::thread::spawn(move || {
        ...
        let got = sub
            .receive_timeout_into(Duration::from_millis(20), &mut buf)
            .unwrap();
