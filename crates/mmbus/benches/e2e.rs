...
fn setup_e2e(label: &str, slot_size: u32) -> E2EBench {
    ...
    std::thread::spawn(move || {
        let mut sub = Subscriber::connect(&label_owned, &cfg_sub, Duration::from_secs(10)).unwrap();
    ...
    pub_.wait_for_subscribers(1, Duration::from_secs(10))
        .unwrap();
...
fn e2e_throughput(c: &mut Criterion) {
    ...
    for msg_size in [32usize, 256] {
        ...
        group.bench_with_input(BenchmarkId::new("msg_size", msg_size), &msg_size, |b, _| {
            b.iter_custom(|iters| {
