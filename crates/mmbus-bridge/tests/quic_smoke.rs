...
fn build_quic_listener(
    ...
    let fp = b
        .quic_fingerprint
        .clone()
        .expect("quic_fingerprint populated");
    (b, addr, fp)
}
...
fn quic_forward_then_receive_end_to_end() {
    ...
    let (recv_bridge, recv_addr, recv_fp) = build_quic_listener(tmp_recv.path(), "quic-recv", psk);
    ...
    let send_bridge = build_quic_dialer(tmp_send.path(), "quic-send", recv_addr, &recv_fp, psk);
...
fn quic_cert_mismatch_blocks_forward() {
    ...
    let pub_thread = thread::spawn(move || {
        ...
        if bus
            .wait_for_subscribers("events", 1, Duration::from_secs(2))
            .is_ok()
        {
            for i in 0..3u64 {
