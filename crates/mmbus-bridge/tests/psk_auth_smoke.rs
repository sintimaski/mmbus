...
fn wrong_psk_is_dropped_and_messages_not_republished() {
    ...
    let receiver = thread::spawn(move || {
        ...
        // assertion.
        let result = bus.subscribe_with_history_timeout("events", 100, Duration::from_millis(800));
        match result {
            Err(mmbus::Error::Timeout(_)) => Ok(()),
            Ok(mut sub) => {
                if let Some(msg) = sub.recv_timeout(Duration::from_millis(500)).unwrap_or(None) {
                    Err(format!(
