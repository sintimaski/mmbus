...
impl Bridge {
    ...
    pub fn start(cfg: &BridgeConfig) -> Result<Self, BridgeError> {
        ...
        let (rx_threads, publish_thread, publish_tx) = if want_tcp_listen || want_quic_listen {
            ...
            if let Some(listen_str) = &cfg.listen {
                let listener =
                    TcpListener::bind(listen_str.as_str()).map_err(BridgeError::Listen)?;
                listener
                    .set_nonblocking(true)
                    .map_err(BridgeError::Listen)?;
                let resolved = listener.local_addr().map_err(BridgeError::Listen)?;
            ...
            if let Some(listen_str) = &cfg.listen_quic {
                let bind_addr: std::net::SocketAddr = listen_str.parse().map_err(|e| {
                    BridgeError::QuicSetup(format!("listen_quic={listen_str:?}: {e}"))
                })?;
                let id = quic_identity.as_ref().ok_or_else(|| {
                    BridgeError::QuicSetup("identity missing for QUIC listener".to_owned())
                })?;
                let server_config =
                    crate::quic::server_config_from_identity(id).map_err(quic_err_to_bridge)?;
                let rt = quic_runtime
                    .as_ref()
                    .expect("runtime must exist if QUIC requested");
    ...
    while !shutdown.load(Ordering::Acquire) {
        let stream = match TcpStream::connect_timeout(
            ...
            Err(e) => {
                ...
                eprintln!(
                    "bridge: peer {peer_name:?} connect failed ({e}); retrying in {backoff:?}"
                );
...
fn run_connection(
    ...
    loop {
        ...
        match rx.recv_timeout(Duration::from_millis(200)) {
            ...
            None => {
                if shutdown.load(Ordering::Acquire) {
                    eprintln!("bridge: peer {peer_name:?} channel closed; closing TCP cleanly");
                    return Ok(());
                }
            ...
            match decode(&buf) {
                Ok((frame, n)) => {
                    if !authenticated {
                        ...
                        eprintln!("bridge: authenticated peer origin_id={}", hello.origin_id);
                        authenticated = true;
                        buf.drain(..n);
                        continue;
                    }
...
fn publisher_main(
    ...
    for topic in &receive_topics {
        if let Err(e) = bus.wait_for_subscribers(topic, 0, Duration::from_millis(0)) {
            eprintln!("bridge: could not pre-create producer for receive topic {topic:?}: {e}");
        }
    }
    while !shutdown.load(Ordering::Acquire) {
        match rx.recv_timeout(Duration::from_millis(200)) {
...
> {
    ...
    let id = if want_listen {
        let base = cfg.base_dir.clone().unwrap_or_else(std::env::temp_dir);
        let cert_path = cfg
            .quic_cert_path
            .clone()
        ...
        Some(crate::quic::gen_or_load_identity(&cert_path, &key_path).map_err(quic_err_to_bridge)?)
    } else {
        None
    };
