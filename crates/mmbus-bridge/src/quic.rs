...
impl Identity {
    ...
    pub fn generate() -> Result<Self, QuicError> {
        ...
        let mut params =
            rcgen::CertificateParams::new(vec!["mmbus-bridge".to_owned(), "localhost".to_owned()])
    ...
    pub fn load(cert_path: &Path, key_path: &Path) -> Result<Self, QuicError> {
        let cert_der = std::fs::read(cert_path).map_err(|e| QuicError::IdentityIo {
            path: cert_path.to_owned(),
            source: e,
        })?;
        let key_der = std::fs::read(key_path).map_err(|e| QuicError::IdentityIo {
    ...
    pub fn save(&self, cert_path: &Path, key_path: &Path) -> Result<(), QuicError> {
        std::fs::write(cert_path, &self.cert_der).map_err(|e| QuicError::IdentityIo {
            path: cert_path.to_owned(),
            source: e,
        })?;
        std::fs::write(key_path, &self.key_der).map_err(|e| QuicError::IdentityIo {
            path: key_path.to_owned(),
            source: e,
        })?;
        #[cfg(unix)]
        {
            ...
            let mut perms = std::fs::metadata(key_path)
                .map_err(|e| QuicError::IdentityIo {
                    path: key_path.to_owned(),
                    source: e,
                })?
                .permissions();
            perms.set_mode(0o600);
            std::fs::set_permissions(key_path, perms).map_err(|e| QuicError::IdentityIo {
                path: key_path.to_owned(),
                source: e,
            })?;
        }
...
pub fn gen_or_load_identity(cert_path: &Path, key_path: &Path) -> Result<Identity, QuicError> {
...
pub fn client_config_pinning(expected_fp: String) -> Result<quinn::ClientConfig, QuicError> {
...
pub fn server_config_from_identity(id: &Identity) -> Result<quinn::ServerConfig, QuicError> {
                ...
                Ok((frame, n)) => {
                    if !authenticated {
                        ...
                        let parsed = match crate::frame::parse_peer_hello(&frame.payload) {
                            Ok(p) => p,
                            Err(e) => {
                                eprintln!("bridge: QUIC malformed PeerHello: {e}; closing");
                                return;
                            }
                        };
