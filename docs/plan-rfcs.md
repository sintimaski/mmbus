...
| **B4b** | QUIC transport (quinn) behind a `quic` Cargo feature; self-signed peer-pinned certs (SSH known_hosts model); TLS 1.3 via rustls; ALPN `mmbus-bridge/1`; PSK auth on the first QUIC stream (identical to B4a's TCP path); shared publisher thread across TCP + QUIC listeners | shipped (B4b-1 thru B4b-5) — see [`rfc-b4b-quic.md`](rfc-b4b-quic.md) for the design |
